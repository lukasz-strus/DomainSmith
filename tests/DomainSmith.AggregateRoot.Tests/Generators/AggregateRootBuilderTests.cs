using DomainSmith.Abstraction.Common.Models;
using DomainSmith.AggregateRoot.Generators;
using FluentAssertions;

namespace DomainSmith.AggregateRoot.Tests.Generators;

public sealed class AggregateRootBuilderTests
{
    [Fact]
    public void Build_WithTwoProperties_ShouldGenerateClassWithCtorCreateUpdateHooks()
    {
        // Arrange
        var builder = new AggregateRootBuilder();
        builder.SetUsings(["using System;"]);
        builder.SetNamespace("TestNamespace");
        builder.SetClassName("Cart");
        builder.SetTypeAgr("Guid");
        builder.SetIdValue("Guid.NewGuid()");
        builder.SetProperties([
            CreatePropertyInfo("string", "Title", false),
            CreatePropertyInfo("int", "ItemsCount", false)
        ]);
        builder.SetIsResultPattern(false);

        // Act
        var output = builder.Build();

        // Assert
        output.Should().NotBeNullOrWhiteSpace();
        output.Should().Contain("// <auto-generated/>");
        output.Should().Contain("namespace TestNamespace;");
        output.Should().Contain("partial class Cart : AggregateRoot<Guid>");
        output.Should().Contain("private Cart(Guid id, string title, int itemscount) : base(id)");
        output.Should().Contain("Title = title;");
        output.Should().Contain("ItemsCount = itemscount;");
        output.Should().Contain("public static Cart? Create(string title, int itemscount)");
        output.Should().Contain("bool canCreate = true;");
        output.Should().Contain("OnCreatingAuto(ref title, ref itemscount, ref canCreate);");
        output.Should().Contain("OnCreating(ref title, ref itemscount, ref canCreate);");
        output.Should().Contain("var result = new Cart(Guid.NewGuid(), title, itemscount);");
        output.Should().Contain("OnCreated(result, ref canCreate);");
        output.Should().Contain("if(!canCreate) return null;");
        output.Should().Contain("return result;");
        output.Should().Contain("public void Update(string title, int itemscount)");
        output.Should().Contain("bool canUpdate = true;");
        output.Should().Contain("OnUpdatingAuto(ref title, ref itemscount, ref canUpdate);");
        output.Should().Contain("OnUpdating(ref title, ref itemscount, ref canUpdate);");
        output.Should().Contain("var tmpTitle = Title;");
        output.Should().Contain("var tmpItemsCount = ItemsCount;");
        output.Should().Contain("Title = title;");
        output.Should().Contain("ItemsCount = itemscount;");
        output.Should().Contain("OnUpdated(ref canUpdate);");
        output.Should().Contain("if(!canUpdate)");
        output.Should().Contain("Title = tmpTitle;");
        output.Should().Contain("ItemsCount = tmpItemsCount;");
        output.Should().Contain("static partial void OnCreatingTitle(ref string title, ref bool canCreate);");
        output.Should().Contain("static partial void OnCreatingItemsCount(ref int itemscount, ref bool canCreate);");
        output.Should().Contain("partial void OnUpdatingTitle(ref string title, ref bool canUpdate);");
        output.Should().Contain("partial void OnUpdatingItemsCount(ref int itemscount, ref bool canUpdate);");
        output.Should().Contain("OnCreatingTitle(ref title, ref canCreate);");
        output.Should().Contain("OnCreatingItemsCount(ref itemscount, ref canCreate);");
        output.Should().Contain("OnUpdatingTitle(ref title, ref canUpdate);");
        output.Should().Contain("OnUpdatingItemsCount(ref itemscount, ref canUpdate);");

        builder.Clear();
    }

    [Fact]
    public void Build_WithAutoGeneratedProperty_ShouldIncludeCreateAndUpdateAutoMethodsCalls()
    {
        // Arrange
        var builder = new AggregateRootBuilder();
        builder.SetUsings(["using System;"]);
        builder.SetNamespace("AutoNamespace");
        builder.SetClassName("Order");
        builder.SetTypeAgr("int");
        builder.SetIdValue("0");
        builder.SetProperties([
            CreatePropertyInfo("decimal", "Total", false),
            CreatePropertyInfo("DateTime", "CreatedAt", true)
        ]);
        builder.SetIsResultPattern(false);

        // Act
        var output = builder.Build();

        // Assert
        output.Should().NotBeNullOrWhiteSpace();
        output.Should().Contain("namespace AutoNamespace;");
        output.Should().Contain("partial class Order : AggregateRoot<int>");
        output.Should().Contain("private Order(int id, decimal total) : base(id)");
        output.Should().Contain("Total = total;");
        output.Should().Contain("public static Order? Create(decimal total)");
        output.Should().Contain("var result = new Order(0, total);");
        output.Should().Contain("result.CreateCreatedAt();");
        output.Should().Contain("partial void CreateCreatedAt();");
        output.Should().Contain("public void Update(decimal total)");
        output.Should().Contain("UpdateCreatedAt();");
        output.Should().Contain("partial void UpdateCreatedAt();");

        builder.Clear();
    }

    [Fact]
    public void Build_WithTwoPropertiesAndResultPattern_ShouldGenerateClassWithCtorCreateUpdateHooks()
    {
        // Arrange
        var builder = new AggregateRootBuilder();
        builder.SetUsings(["using System;"]);
        builder.SetNamespace("TestNamespace");
        builder.SetClassName("Cart");
        builder.SetTypeAgr("Guid");
        builder.SetIdValue("Guid.NewGuid()");
        builder.SetProperties([
            CreatePropertyInfo("string", "Title", false),
            CreatePropertyInfo("int", "ItemsCount", false)
        ]);
        builder.SetIsResultPattern(true);

        // Act
        var output = builder.Build();

        // Assert
        output.Should().NotBeNullOrWhiteSpace();
        output.Should().Contain("// <auto-generated/>");
        output.Should().Contain("using DomainSmith.Abstraction.Core.Result;");
        output.Should().Contain("namespace TestNamespace;");
        output.Should().Contain("partial class Cart : AggregateRoot<Guid>");
        output.Should().Contain("private Cart(Guid id, string title, int itemscount) : base(id)");
        output.Should().Contain("Title = title;");
        output.Should().Contain("ItemsCount = itemscount;");
        output.Should().Contain("public static Result<Cart> Create(string title, int itemscount)");
        output.Should().Contain("bool canCreate = true;");
        output.Should().Contain("Error error = Error.None;");
        output.Should().Contain("OnCreatingAuto(ref title, ref itemscount, ref canCreate, ref error);");
        output.Should().Contain("OnCreating(ref title, ref itemscount, ref canCreate, ref error);");
        output.Should().Contain("var result = new Cart(Guid.NewGuid(), title, itemscount);");
        output.Should().Contain("OnCreated(result, ref canCreate, ref error);");
        output.Should().Contain("if(!canCreate) return Result.Failure<Cart>(error);");
        output.Should().Contain("return Result.Success(result);");
        output.Should().Contain("public Result<bool> Update(string title, int itemscount)");
        output.Should().Contain("bool canUpdate = true;");
        output.Should().Contain("Error error = Error.None;");
        output.Should().Contain("OnUpdatingAuto(ref title, ref itemscount, ref canUpdate, ref error);");
        output.Should().Contain("OnUpdating(ref title, ref itemscount, ref canUpdate, ref error);");
        output.Should().Contain("var tmpTitle = Title;");
        output.Should().Contain("var tmpItemsCount = ItemsCount;");
        output.Should().Contain("Title = title;");
        output.Should().Contain("ItemsCount = itemscount;");
        output.Should().Contain("OnUpdated(ref canUpdate, ref error);");
        output.Should().Contain("if(!canUpdate)");
        output.Should().Contain("Title = tmpTitle;");
        output.Should().Contain("ItemsCount = tmpItemsCount;");
        output.Should().Contain("return Result.Failure<bool>(error);");
        output.Should().Contain("return Result.Success(true);");
        output.Should()
            .Contain(
                "private static void OnCreatingAuto(ref string title, ref int itemscount, ref bool canCreate, ref Error error)");
        output.Should()
            .Contain(
                "private void OnUpdatingAuto(ref string title, ref int itemscount, ref bool canUpdate, ref Error error)");
        output.Should()
            .Contain(
                "static partial void OnCreating(ref string title, ref int itemscount, ref bool canCreate, ref Error error);");
        output.Should().Contain("static partial void OnCreated(Cart result, ref bool canCreate, ref Error error);");
        output.Should()
            .Contain(
                "partial void OnUpdating(ref string title, ref int itemscount, ref bool canUpdate, ref Error error);");
        output.Should().Contain("partial void OnUpdated(ref bool canUpdate, ref Error error);");
        output.Should()
            .Contain("static partial void OnCreatingTitle(ref string title, ref bool canCreate, ref Error error);");
        output.Should()
            .Contain(
                "static partial void OnCreatingItemsCount(ref int itemscount, ref bool canCreate, ref Error error);");
        output.Should().Contain("partial void OnUpdatingTitle(ref string title, ref bool canUpdate, ref Error error);");
        output.Should()
            .Contain("partial void OnUpdatingItemsCount(ref int itemscount, ref bool canUpdate, ref Error error);");
        output.Should().Contain("OnCreatingTitle(ref title, ref canCreate, ref error);");
        output.Should().Contain("OnCreatingItemsCount(ref itemscount, ref canCreate, ref error);");
        output.Should().Contain("OnUpdatingTitle(ref title, ref canUpdate, ref error);");
        output.Should().Contain("OnUpdatingItemsCount(ref itemscount, ref canUpdate, ref error);");

        builder.Clear();
    }

    [Fact]
    public void Build_WithAutoGeneratedPropertyAndResultPattern_ShouldIncludeCreateAndUpdateAutoMethodsCalls()
    {
        // Arrange
        var builder = new AggregateRootBuilder();
        builder.SetUsings(["using System;"]);
        builder.SetNamespace("AutoNamespace");
        builder.SetClassName("Order");
        builder.SetTypeAgr("int");
        builder.SetIdValue("0");
        builder.SetProperties([
            CreatePropertyInfo("decimal", "Total", false),
            CreatePropertyInfo("DateTime", "CreatedAt", true)
        ]);
        builder.SetIsResultPattern(true);

        // Act
        var output = builder.Build();

        // Assert
        output.Should().NotBeNullOrWhiteSpace();
        output.Should().Contain("namespace AutoNamespace;");
        output.Should().Contain("partial class Order : AggregateRoot<int>");
        output.Should().Contain("private Order(int id, decimal total) : base(id)");
        output.Should().Contain("Total = total;");
        output.Should().Contain("public static Result<Order> Create(decimal total)");
        output.Should().Contain("var result = new Order(0, total);");
        output.Should().Contain("result.CreateCreatedAt();");
        output.Should().Contain("partial void CreateCreatedAt();");
        output.Should().Contain("public Result<bool> Update(decimal total)");
        output.Should().Contain("UpdateCreatedAt();");
        output.Should().Contain("partial void UpdateCreatedAt();");

        builder.Clear();
    }

    [Fact]
    public void Build_WithMissingInputs_ShouldReturnEmptyString()
    {
        // Arrange
        var builder = new AggregateRootBuilder();

        // Act
        var output = builder.Build();

        // Assert
        output.Should().BeEmpty();
    }

    private static PropertyInfo CreatePropertyInfo(string type, string name, bool autoGenerated)
        => new(type, name, autoGenerated);
}